...
const vitepressDir = resolve(__dirname, '../');
const rootDocsDir = resolve(vitepressDir, '../');
const rootDir = resolve(rootDocsDir, '../');
const srcDir = resolve(rootDir, 'src');
const typesDir = resolve(rootDir, 'dist', 'types');

const sidebarFile = resolve(vitepressDir, 'functions.ts');
...
${byCategory
  .map(
    item =>
      `### ${item.name}\n\n${item.files
        .map(file => `[${file.baseDir}](/functions/${file.baseDir}/)`)
        .join('    \n')}`
  )
  .join('\n\n')}
`;
